...
void MIxnyn(int dimx, int dimy, int K, int N, char* fname, double *arr, double *MI) 
{
  FILE *fin;
  int i;
  double **x;
  ...
  // Convert data from matlab array to expected format
  c = 0;
  for (i=0; i<N; i++) {
  ...
  for (d=0;d<dimx+dimy;d++) free(x[d]); 
  free(x);
  free(scal);
  free(min);free(max);
  free(psi);
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
    ...
    void MIxnyn(int, int, int, int, char *, double *, double *);
    ...
    MIxnyn(dimx, dimy, K, N, fname, arr, MI) ;
